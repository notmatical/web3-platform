    ...
    for (let i = 0; i < userPool.itemCount.toNumber(); i++) {
        if (userPool.items[i].nftAddr.toBase58() === nftMint.toBase58()) {
            if (userPool.items[i].model.toNumber() === 1 &&  now < userPool.items[i].lockTime.toNumber()) {
                reward = (Math.floor((now - userPool.items[i].rewardTime.toNumber()) / EPOCH)) * (userPool.items[i].rate.toNumber() / LAMPORTS_PER_SOL) * 0.75;
            } else {
...
export const claimRewardAll = async (
    ...
    try {
        ...
        // showInfoToast('Claim successful');
        stopLoading();
        updatePage();
    } catch (error) {
        console.log(error);
...
export const claimReward = async (
    ...
    try {
        ...
        // showInfoToast('Claim has been successfully processed.');
        stopLoading();
        updatePage();
    } catch (error) {
        console.log(error);
