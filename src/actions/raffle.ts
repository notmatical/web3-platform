...
import { DECIMALS, GLOBAL_AUTHORITY_SEED, PREY_DECIMALS, COSMIC_TOKEN_MINT, RAFFLE_PROGRAM, RAFFLE_SIZE, RAFFLE_ADMINS } from 'config';
import { solConnection, getAssociatedTokenAccount, getATokenAccountsNeedCreate } from 'actions/shared';
import { WalletContextState } from '@solana/wallet-adapter-react';

// utility
export const adminValidation = (wallet: PublicKey) => {
    let res = false;
    if (wallet === null) return false;
    const address = wallet.toBase58();
    for (let item of RAFFLE_ADMINS) {
        res = res || (item.address === address)
    }
    return res
}
...
export const createRaffle = async (
    ...
    try {
        ...
        tx.add(program.instruction.createRaffle(
            bump,
            new anchor.BN(ticketPriceCosmic * PREY_DECIMALS),
...
export const buyTicket = async (
    ...
    try {
        ...
        let userTokenAccount = await getAssociatedTokenAccount(userAddress, COSMIC_TOKEN_MINT);
...
export const claimReward = async (
    ...
    try {
        const srcCosmicTokenAccount = await getAssociatedTokenAccount(globalAuthority, COSMIC_TOKEN_MINT);
        const claimerCosmicTokenAccount = await getAssociatedTokenAccount(userAddress, COSMIC_TOKEN_MINT);
        ...
        tx.add(program.instruction.claimReward(
            bump,
            {
                accounts: {
                    ...
                    srcNftTokenAccount,
                    srcCosmicTokenAccount,
                    claimerCosmicTokenAccount,
                    nftMintAddress: nft_mint,
                    tokenProgram: TOKEN_PROGRAM_ID,
                },
                instructions: [],
